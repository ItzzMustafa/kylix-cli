...
use kylix_pqc::ml_dsa::{MlDsa44, MlDsa65, MlDsa87};
use rand::rngs::OsRng;
...
fn bench_ml_dsa_44(c: &mut Criterion) {
    ...
    group.bench_function("keygen", |b| {
        b.iter(|| MlDsa44::keygen(&mut OsRng).unwrap())
    });

    let (sk, vk) = MlDsa44::keygen(&mut OsRng).unwrap();
...
fn bench_ml_dsa_65(c: &mut Criterion) {
    ...
    group.bench_function("keygen", |b| {
        b.iter(|| MlDsa65::keygen(&mut OsRng).unwrap())
    });

    let (sk, vk) = MlDsa65::keygen(&mut OsRng).unwrap();
...
fn bench_ml_dsa_87(c: &mut Criterion) {
    ...
    group.bench_function("keygen", |b| {
        b.iter(|| MlDsa87::keygen(&mut OsRng).unwrap())
    });

    let (sk, vk) = MlDsa87::keygen(&mut OsRng).unwrap();
